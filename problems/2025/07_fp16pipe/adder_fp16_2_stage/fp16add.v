...
reg                   special;
reg [`FP16_WIDTH-1:0] special_res;

reg                   bypass;
reg [`FP16_WIDTH-1:0] bypass_res;
...
reg                   special_s1;
reg [`FP16_WIDTH-1:0] special_res_s1;

reg                   bypass_s1;
reg [`FP16_WIDTH-1:0] bypass_res_s1;

reg sign_big_s1;
reg sign_small_s1;

reg [`FP16_EXPONENT_WIDTH-1:0] exp_big_s1;

reg [`FP16_MANTISSA_WIDTH+1:0] mant_big_ext_s1;
reg [`FP16_MANTISSA_WIDTH+1:0] mant_shifted_s1;
...
always @(*) begin
    ...
    special     = 1'b0;
    special_res = {`FP16_WIDTH{1'b0}};

    bypass     = 1'b0;
    bypass_res = {`FP16_WIDTH{1'b0}};

    if ((&exp_a && |frac_a) || (&exp_b && |frac_b)) begin // nan
        special     = 1'b1;
        special_res = {1'b0, {`FP16_EXPONENT_WIDTH{1'b1}}, {`FP16_MANTISSA_WIDTH{1'b1}}};
    end
        ...
        if (sign_a != sign_b) // +inf + -inf
            ...
            if (&exp_a && (frac_a == 0))
                special_res = {sign_a, {`FP16_EXPONENT_WIDTH{1'b1}}, {`FP16_MANTISSA_WIDTH{1'b0}}};
            else
                special_res = {sign_b, {`FP16_EXPONENT_WIDTH{1'b1}}, {`FP16_MANTISSA_WIDTH{1'b0}}};
        end
    end
        ...
        special     = 1'b1;
        special_res = {sign_b, exp_b, frac_b};
    end

    else if (exp_b == 0) begin
        special     = 1'b1;
        special_res = {sign_a, exp_a, frac_a};
    end
        ...
        if (exp_diff >= (`FP16_MANTISSA_WIDTH + 2)) begin
            bypass     = 1'b1;
...
always @(posedge clk) begin
    ...
    special_s1     <= special;
    special_res_s1 <= special_res;

    bypass_s1     <= bypass;
    bypass_res_s1 <= bypass_res;
end
...
always @(*) begin
    ...
    if (special_s1)
        o_res = special_res_s1;
    else if (bypass_s1)
        o_res = bypass_res_s1;
    else begin
        exp_res  = $signed({1'b0, exp_big_s1});
        sign_res = sign_big_s1;

        if (sign_big_s1 == sign_small_s1) begin
            mant_sum = {1'b0, mant_big_ext_s1} + {1'b0, mant_shifted_s1};
            ...
            mant_res = mant_big_ext_s1 - mant_shifted_s1;
