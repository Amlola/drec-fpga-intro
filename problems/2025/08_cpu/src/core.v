...
reg        wb_en_f1_stage;
reg [4:0]  rd_addr_f1_stage;
reg [31:0] alu_res_f1_stage;
reg [29:0] pc_inc_f1_stage;
reg [31:0] imm_u_f1_stage;
reg [1:0]  wb_sel_f1_stage;

wire bypass_en = wb_en_f1_stage && (wb_sel_f1_stage != 2'b01);
...
mux4 #(
    .WIDTH(32)
) mux4_sel_bypass_inst (
    .i0    (alu_res_f1_stage),
    .i1    ({32{1'b0}}),
    .i2    ({pc_inc_f1_stage, {2{1'b0}}}),
    .i3    (imm_u_f1_stage),
    .i_sel (wb_sel_f1_stage),
    .o_y   (bypass_data)
);
...
regfile #(
    ...
    .o_rd1_data    (rs1_data),
    .o_rd2_data    (rs2_data),
    .i_wr_addr     (rd_addr_f1_stage),
    .i_wr_data     (wr_data),
    .i_wr_en       (wb_en_f1_stage),
    .i_bypass_en   (bypass_en),
    .i_bypass_data (bypass_data)
);
...
wire [31:0] lsu_addr = load ? (rs1_data + imm_i) : store ? (rs1_data + imm_s) : 32'd0;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        pc        <= pc_next;
        is_second <= load && !is_second;
    end
end

always @(posedge clk) begin
    wb_en_f1_stage   <= wb_en;
    rd_addr_f1_stage <= rd_addr;
    alu_res_f1_stage <= alu_res;
    pc_inc_f1_stage  <= pc_inc;
    imm_u_f1_stage   <= imm_u;
    wb_sel_f1_stage  <= wb_sel;
end
