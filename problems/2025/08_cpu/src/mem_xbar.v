...
wire i_addr_is_dmem = (i_addr >= DATA_START) && (i_addr <= DATA_LIMIT);
wire i_addr_is_mmio = (i_addr >= MMIO_START) && (i_addr <= MMIO_LIMIT);

wire addr_data_is_dmem = (addr_data >= DATA_START) && (addr_data <= DATA_LIMIT);
wire addr_data_is_mmio = (addr_data >= MMIO_START) && (addr_data <= MMIO_LIMIT);
...
always @(*) begin
    ...
    if (i_addr_is_dmem) begin
        o_dmem_addr = i_addr - DATA_START;
        o_dmem_data = i_data;
        o_dmem_wren = i_wren;
        o_dmem_mask = i_mask;
    end else if (i_addr_is_mmio) begin
    ...
    if (addr_data_is_dmem)
        o_data = i_dmem_data;
    else if (addr_data_is_mmio)
        o_data = i_mmio_data;
end
